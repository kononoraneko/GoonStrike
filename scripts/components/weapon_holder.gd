...
func _broadcast_reload_state(is_reloading: bool, duration: float) -> void:
	_apply_reload_animation(is_reloading, duration)
...
func _set_weapon(data: WeaponData) -> void:
	...
	instance.reload_state_changed.connect(_on_reload_state_changed)
...
func _on_reload_state_changed(is_reloading: bool) -> void:
	if current_weapon == null or current_weapon.data == null:
		return
	var duration := max(current_weapon.data.reload_time, 0.01)
	_apply_reload_animation(is_reloading, duration)
	if multiplayer.is_server():
		_sync_weapon_ammo(owner_player.remote_player_id)
		_broadcast_reload_state.rpc(is_reloading, duration)


func _apply_reload_animation(is_reloading: bool, duration: float) -> void:
	if owner_player == null or owner_player.animation == null:
		return
	if is_reloading:
		owner_player.animation.play_reload(duration)
	else:
		owner_player.animation.stop_reload()
